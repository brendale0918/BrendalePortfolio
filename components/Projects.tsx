...
export default function Projects() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-gradient-to-b from-[#1a1a1a] to-black">
      ...
      {/* Main Content */}
<section className="pt-28 px-4 sm:px-6 lg:px-8 pb-16">
  {/* Title Section */}
  <div className="max-w-7xl mx-auto pt-4 pb-6">
    <h2 className="text-3xl md:text-4xl font-extrabold text-[#ff6a00] mb-8 text-center">
      My Projects
    </h2>
  </div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 relative z-20">
    {projects.map((project) => {
      ...
      return (
        <motion.div
          key={project.id}
          className="flex flex-col items-center justify-center bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6 shadow-lg relative overflow-hidden hover:scale-[1.02] transition-transform"
        ...
        >
          ...
          {/* Title */}
          <h3 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-[#ff6a00] to-[#ff9e42] bg-clip-text text-transparent text-center relative z-10">
            {project.title}
          </h3>
          ...
          {/* Carousel */}
          <div className="relative w-full h-[18rem] sm:h-[22rem] md:h-[26rem] flex items-center justify-center overflow-hidden z-10">
            <div
              ...
              {project.images.map((img, i) => (
                ...
                >
                  <img
                    ...
                    className="w-full h-[18rem] sm:h-[22rem] md:h-[26rem] object-contain rounded-xl shadow-lg"
                  />
            ...
            {/* Prev Button */}
            <button
              className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-orange-500/20 backdrop-blur-md text-white text-2xl sm:text-3xl shadow-lg hover:bg-orange-500/40 hover:scale-110 transition-all duration-300 z-20"
            ...
            {/* Next Button */}
            <button
              className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full bg-orange-500/20 backdrop-blur-md text-white text-2xl sm:text-3xl shadow-lg hover:bg-orange-500/40 hover:scale-110 transition-all duration-300 z-20"
...
{zoomState && (
  <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80">
    <div className="relative flex items-center justify-center w-full h-full px-4">
      {/* Prev button */}
      <button
        className="absolute left-4 sm:left-6 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-14 sm:h-14 flex items-center justify-center rounded-full bg-orange-500/30 backdrop-blur-md text-white text-2xl sm:text-3xl shadow-lg hover:bg-orange-500/60 hover:scale-110 transition-all"
      ...
      <img
        ...
        alt="Zoomed"
        className="max-h-[70vh] sm:max-h-[80vh] max-w-[95vw] object-contain rounded-lg shadow-2xl"
      />

      {/* Next button */}
      <button
        className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-14 sm:h-14 flex items-center justify-center rounded-full bg-orange-500/30 backdrop-blur-md text-white text-2xl sm:text-3xl shadow-lg hover:bg-orange-500/60 hover:scale-110 transition-all"
    ...
    <div className="mt-6 flex gap-4 flex-wrap justify-center">
      <button
        className="px-4 sm:px-6 py-2 bg-orange-500 text-white font-semibold rounded-lg shadow hover:bg-orange-600 transition"
      ...
      <a
        ...
        className="px-4 sm:px-6 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow hover:bg-blue-600 transition"
      >
        Open Full Size
      </a>
    </div>
